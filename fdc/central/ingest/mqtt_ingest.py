...
topic ที่ ศอย. (uplink.py) ส่งขึ้นมา ภายใต้ base = fdc/<unit>/<node> :
    ...
    .../target/<id>         เป้าที่ปักหมุดไว้ (ยังไม่ยิง) — retained; active=false = ลบ
...
async def _handle(topic: str, payload: bytes):
    ...
    elif len(rest) == 2 and rest[0] == "target":
        kind = "target"
        data["id"] = rest[1]               # topic เป็นตัวกำหนด id (เชื่อถือได้กว่า payload)
        await db.upsert_target(unit, node, rest[1], ts, data)
